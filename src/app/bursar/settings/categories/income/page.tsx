...
export default function IncomeCategoryPage() {
    const { incomeCategories, updateIncomeCategory, addIncomeCategory, deleteIncomeCategory } = useSchoolData();
    ...
    const handleAdd = () => {
        ...
        if (addIncomeCategory) {
            addIncomeCategory({ name: newCategoryName, subcategories: [] } as any);
        }
        setNewCategoryName("");
        setIsAdding(false);
    };
    ...
    const handleSaveEdit = (updated: TransactionCategoryItem) => {
        if (updateIncomeCategory) {
            updateIncomeCategory(updated);
        }
        setEditingCategory(null);
    };

    const handleDelete = (id: string) => {
        if (deleteIncomeCategory) {
    ...
    return (
        <div className="h-full w-full bg-slate-50 flex flex-col font-sans text-slate-800">
            ...
            <div className="bg-white px-4 py-3 border-b border-slate-200 shadow-sm flex items-center justify-between sticky top-0 z-10">
                ...
                <div className="font-bold text-lg text-slate-800">Income</div>
            ...
            <div className="bg-white px-4 py-3 border-b border-slate-100 flex justify-between items-center">
                ...
                <button
                    ...
                    className={`w-12 h-6 rounded-full relative transition-colors duration-200 ${showSubcategories ? 'bg-blue-400' : 'bg-slate-300'}`}
                >
                    ...
                    {incomeCategories.map((cat, idx) => (
                        <div key={cat.id || idx} className="flex flex-col relative group">
                            <div className="flex items-center p-3 gap-3 hover:bg-slate-50 transition-colors">
                                {/* Delete Icon */}
                                <button onClick={() => deleteIncomeCategory && deleteIncomeCategory(cat.id)} className="text-red-500 hover:text-red-700 transition-colors p-1">
                                ...
                                <div className="flex items-center gap-3 pl-2">
                                    <button onClick={() => setEditingCategory(cat)} className="text-slate-300 hover:text-blue-500 transition-colors">
            ...
            {isAdding && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center animate-fade-in p-4">
                    <div className="bg-white w-full max-w-sm rounded-xl p-4 shadow-2xl animate-slide-up">
                        <h3 className="font-bold text-lg mb-4">Add Income Category</h3>
                        <input
                            autoFocus
                            type="text"
