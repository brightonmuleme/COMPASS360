...
export default function AdmissionsPage() {
    ...
    const handleRemovePaymentRow = (index: number) => {
        setLocalFormData(prev => {
            ...
            // Sync legacy schoolPayCode to the new first item
            const overrides: any = {};
            if (updated.length > 0) {
                overrides.schoolPayCode = updated[0].code;
            } else {
                overrides.schoolPayCode = '';
            }
    ...
    const handlePaymentCodeBlur = (index: number, value: string) => {
        const sanitized = value.trim().toUpperCase();
        if (sanitized !== value) {
            handlePaymentDetailChange(index, 'code', sanitized);
        }
    };
    ...
    const processCSV = (csvText: string) => {
        ...
        // 1. Identify Header Row (Look for keywords in first few lines)
        let headerIndex = -1;
        let headers: string[] = [];

        for (let i = 0; i < Math.min(lines.length, 5); i++) {
            const row = lines[i].toLowerCase();
            if (row.includes('name') && (row.includes('code') || row.includes('pay'))) {
                headerIndex = i;
        ...
        // Fallback: Assume row 0 if no header detected, but try to parse it anyway
        if (headerIndex === -1) {
            headerIndex = 0;
        ...
        const idxName = getIdx(['name', 'student']);
        const idxFirstName = getIdx(['first']);
        const idxLastName = getIdx(['last']);
        const idxPayCode = getIdx(['pay', 'code', 'account']);
        const idxGender = getIdx(['gender', 'sex']);
        const idxCourse = getIdx(['course', 'program']);
        const idxLevel = getIdx(['level', 'class', 'entry']);
        const idxDate = getIdx(['date', 'admission']);
        const idxAgent = getIdx(['agent', 'marketing']);
        ...
        for (let i = headerIndex + 1; i < lines.length; i++) {
            ...
            if (idxFirstName !== -1 && idxLastName !== -1) {
                name = `${cols[idxFirstName] || ''} ${cols[idxLastName] || ''}`.trim();
            } else if (idxName !== -1) {
                name = cols[idxName] || "Unknown";
            } else {
                // Fallback to indices 0 and 1
                name = `${cols[0]} ${cols[1]}`;
            }

            // Extract PayCode
            const payCode = idxPayCode !== -1 ? cols[idxPayCode] : cols[2]; // Fallback idx 2

            // Extract Gender (Smarter & Case Insensitive)
            const rawGender = idxGender !== -1 ? cols[idxGender] : (cols[3] || '');
            const gender = ['f', 'female', 'girl', 'woman'].some(s => rawGender.toLowerCase().startsWith(s)) ? 'Female' : 'Male';

            // Other fields with fallbacks
            const course = idxCourse !== -1 ? cols[idxCourse] : cols[4];
            const level = idxLevel !== -1 ? cols[idxLevel] : cols[5];
            const date = idxDate !== -1 ? cols[idxDate] : cols[6];
            const agent = idxAgent !== -1 ? cols[idxAgent] : cols[7];
        ...
        <div style={{ position: 'relative', minHeight: '80vh' }}>
            ...
            <div className="print-only" style={{ marginBottom: '20px', borderBottom: '2px solid black', paddingBottom: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                        <div style={{ fontSize: '20pt', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '1px' }}>{schoolProfile?.name || 'COMPASS 360'}</div>
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 no-print">
                <div>
                    ...
                    <p className="text-sm md:text-base text-slate-400">Manage new and pending admissions.</p>
                </div>

                <div className="flex bg-slate-800/50 p-1 rounded-lg border border-slate-700/50">
                    <button
                        onClick={() => { setViewMode('list'); setSelectedIds([]); }}
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${viewMode === 'list' ? 'bg-purple-600 text-white shadow-lg shadow-purple-900/20' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}
                    >
                        Admitted List
                    </button>
                    <button
                        onClick={() => setViewMode('single')}
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${viewMode === 'single' ? 'bg-purple-600 text-white shadow-lg shadow-purple-900/20' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}
                    >
                        New Admission
                    </button>
                    <button
                        onClick={() => setViewMode('batch')}
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${viewMode === 'batch' ? 'bg-purple-600 text-white shadow-lg shadow-purple-900/20' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}
                    >
                        Batch Upload
                    </button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 md:gap-8">
                    ...
                    <div className="card p-4 md:p-8 animate-fade-in">
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem', position: 'relative' }}>
                            <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: '1px', background: '#334155', zIndex: 0 }}></div>
                            {[1, 2, 3].map(s => (
                                <div key={s} style={{
                                    width: '32px', height: '32px', borderRadius: '50%',
                                    background: step >= s ? '#9333ea' : '#1e293b', // Purple-600 vs Slate-800
                                    color: step >= s ? 'white' : '#94a3b8',
                                    border: step >= s ? 'none' : '2px solid #334155',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                    position: 'relative', zIndex: 1, fontWeight: 'bold', fontSize: '14px',
                        ...
                        <form onSubmit={(e) => e.preventDefault()}>
                            {step === 1 && (
                                <div className="animate-fade-in">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                                        ...
                                        <div className="col-span-1 sm:col-span-2" style={{ background: '#1e293b', padding: '1.25rem', borderRadius: '0.75rem', border: 'none' }}>
                                                ...
                                                <div key={index} className="grid grid-cols-1 sm:grid-cols-[1.5fr_2fr_auto] gap-3 mb-4 items-end">
                                                    <div>
                                                        ...
                                                        <select
                                                            ...
                                                            className="input"
                                                            style={{ width: '100%', padding: '0.7rem', background: '#020617', border: '1px solid #334155', borderRadius: '0.5rem', color: 'white' }}
                                                        >
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            onBlur={(e) => handlePaymentCodeBlur(index, e.target.value)}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2rem', borderTop: '1px solid hsl(var(--border))', paddingTop: '1.5rem' }}>
                                ...
                                {step < 3 ? (
                                    <button onClick={nextStep} className="btn-primary" style={{ padding: '0.75rem 1.5rem', background: '#9333ea', border: 'none', borderRadius: '0.5rem', fontWeight: 'bold' }}>Next Step</button>
                                ) : (
                    ...
                    {/* Summary / Preview - Styled like Enrollment Billing Breakdown */}
                    <div style={{ position: 'sticky', top: '2rem' }} className="h-fit">
                        <div className="card p-5 md:p-8 flex flex-col gap-4 md:gap-6">
                            <h2 style={{ fontSize: '1.2rem', fontWeight: 'bold', textAlign: 'center', margin: 0 }}>Registration Summary</h2>

                            <div className="space-y-2 md:space-y-3 text-sm">
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span className="opacity-70">Name</span>
                                    <span style={{ fontWeight: 'bold' }}>{localFormData.firstName || '...'} {localFormData.lastName}</span>
                                </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    <span style={{ textAlign: 'right' }}>{localFormData.entryLevel}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span className="opacity-70">Pay Code</span>
                                    <span style={{ fontFamily: 'monospace', color: '#f59e0b' }}>{localFormData.schoolPayCode || '...'}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <div style={{ background: 'rgba(59, 130, 246, 0.1)', padding: '1rem', borderRadius: 'var(--radius)', textAlign: 'center' }}>
                                <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '1px', opacity: 0.6, marginBottom: '0.25rem' }}>STATUS</div>
                                <div style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#3b82f6' }}>ADMISSION PENDING</div>
                            </div>

                            <div className="flex flex-col gap-2">
                                <button onClick={() => setViewMode('list')} className="btn btn-outline touch-target py-2.5 text-sm">Cancel Registration</button>
                                <button
                                    onClick={handleRegistration}
                                    className="btn btn-primary touch-target py-2.5 text-sm"
                                    style={{ background: '#22c55e', borderColor: '#22c55e', opacity: step === 3 ? 1 : 0.5 }}
                                    disabled={step !== 3}
                                >
                                    Finish & Save
                                </button>
...
// Inline CSS for the slide animation if not in global
const globalStyles = `
