...
export default function FeesPage() {
    ...
    const hasActivePass = studentProfile.subscriptionStatus === 'active' || (studentProfile.subscriptionExpiry && new Date(studentProfile.subscriptionExpiry) > new Date());
    const isActuallyLogged = hydrated && studentProfile.id !== 'std_user_1';
                ...
                <div className="max-w-6xl mx-auto animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="space-y-4">
                                <h3 className="text-xs font-black uppercase tracking-widest text-gray-500 px-2">Financial History</h3>
                                <div className="space-y-3">
                                    {(() => {
                                        // 1. Map Top-up Requests
                                        const topups = (studentProfile.paymentRequests || []).map(r => ({
                                            id: r.id,
                                            type: 'topup',
                                            label: `TOP-UP: ${r.transactionId}`,
                                            subtext: r.reference,
                                            date: r.submittedAt,
                                            amount: r.amount,
                                            status: r.status,
                                        ...
                                        // 2. Map Plan Purchases
                                        const purchases = (studentProfile.activityLogs || [])
                                            .filter((log: any) => log.type === 'AppPass')
                                            .map((log: any) => ({
                                                id: log.id,
                                                type: 'purchase',
                                                label: `PURCHASE: ${log.plan}`,
                                                subtext: `Platform Pass Activation`,
                                                date: log.timestamp,
                                                amount: log.amount,
                                                status: 'Approved', // Purchases from wallet are always approved immediately
                                                isIncome: false
                                            }));

                                        // 3. Merge and Sort
                                        const history = [...topups, ...purchases].sort((a, b) =>
                                        ...
                                        return history.map((item) => (
                                            <div key={item.id} className="bg-[#111] p-5 rounded-2xl border border-[#222] flex items-center justify-between group hover:border-[#333] transition-colors">
                                                    ...
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${item.status === 'Approved' ? 'bg-green-500/10 text-green-500' :
                                                        ...
                                                        {item.type === 'purchase' ? <Zap size={18} /> :
                                                            item.status === 'Approved' ? <CheckCircle2 size={18} /> :
                                                                item.status === 'Rejected' ? <AlertCircle size={18} /> :
                                                                    <Clock size={18} />}
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    {item.amount && (
                                                        <div className={`text-xs font-bold ${item.isIncome ? 'text-gray-400' : 'text-red-400/60'}`}>
